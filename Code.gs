...
function setup() {
  ...
  if (sheet.getLastRow() === 0) {
    sheet.appendRow(['Timestamp','Team Name','College','Team Leader','Email','Phone','Track','Team Members']);
  }
}
...
function doPost(e) {
  try {
    ...
    sheet.appendRow([new Date(), data.teamName || '', data.college || '', data.leader || '', data.email || '', data.phone || '', data.track || '', data.members || '']);
